...
- objective：`beta=1`，PCG relative tolerance `1e-5`，最多 2048 iterations
...
unpreconditioned CG 保留重复的 `lambda` 特征值，而 Jacobi 会破坏该 Krylov 结构。relative
tolerance 与最多 2048 次 iterations 来自 config；达到 `1e-5` 就提前停止。recursive residual
每 20 次用真实 `rhs-Ax` 重算是 code-locked constant。最终 evidence 始终保存真实 residual；
改变 preconditioner、上限或重算周期必须使用新 Git/config/run identity。
