...
主问题是：**在受限且可能 misspecified 的 reward class 中，直接最小化 policy-relevant
ridge local regret 的 SRM+，能否比 repeated-label BT-MLE 更准确地恢复 operational-oracle
局部 policy update？**
...
[theory.md](theory.md) 第 6 节。

PCG 使用 damped Fisher diagonal 的 Jacobi preconditioner；relative tolerance 与最多 100 次
iterations 来自 config，recursive residual 每 20 次用真实 `rhs-Ax` 重算是 code-locked
constant。最终 evidence 始终保存真实 residual；改变 preconditioner 或重算周期必须使用新
Git/run identity。
...
| 仅 pairwise accuracy 改善 | 任意 | 任意 | 不构成 SRM+ 成功证据 |
