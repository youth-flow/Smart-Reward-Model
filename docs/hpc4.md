...
以下数值仅是**历史 pre-fix 环境基线**，来自 controlled smoke job `1641441`、semantic config
...
但不能为当前 FP64 solver identity 给出 walltime/GPU-hour 预算：

| 资源 | Smoke 实测 | Main 估算依据 | Main 申请值 |
|---|---:|---|---:|
| Wall-clock / seed | `00:03:15` | 当前 FP64 inner solve 必须由 replacement smoke/pilot 重测 | 待 replacement pilot |
| Peak GPU allocated/reserved | `9,376,474,112 / 24,404,557,824 B` | main tensor 增量远低于 L20 47.7 GB，总量仍留显著余量 | 1 × L20 |
| Host MaxRSS / allocated CPUs | `3,906,312 KiB / 16` | site `DefCpuPerGPU=16`，实际 AllocTRES memory `120G` | 保持同一 site default |
| Scratch peak | `21,926,294 B` | Phase-1 artifact 主体近似按 prompts 32x 放大 | 约 `0.75 GB/seed` |
| Project persistent bytes / seed | `18,467,909 B` artifact + `1,574,291 B` run directory | 同上并计入 rollout/comparison | 约 `0.65 GB/seed` |
| GPU-hours / 5 seeds | smoke 仅 `0.054 h` | pre-fix `30 GPU-h` 估算已失效 | replacement 实测后填写；peak `2` |

三项 storage 实测统一采用 GNU
...
只有 replacement FP64 smoke/pilot 的新身份数值符合 quota/account 预算，才能确定 main walltime。
...
QoS，因此 replacement campaign 取 2；`MaxJobsPU=2` 仍是所有 split arrays 的全局并发上限。
