...
以下资源基线来自已验收 controlled smoke：job `1641441`、semantic config
...
|---|---:|---|---:|
| Wall-clock / seed | `00:03:15` | materialize `~1:45`；compare `~0:08`；rollout `~0:52`；main 还含 32x prompts、72x steps、三档 damping | `12:00:00` hard limit |
...
| GPU-hours / 5 seeds | smoke 仅 `0.054 h` | 预计约 `30 GPU-h`；walltime hard ceiling 为 `60 GPU-h` | 每个 array concurrency `1`；campaign peak `2` |
...
单 index 或连续闭区间，`PRORM_ARRAY_CONCURRENCY` 必须是正整数；没有明确预算时保持 1。
