...
仓库不负责猜测适合 HPC4 驱动的 CUDA image。**当前仓库尚未包含经 HPC4 验证的
Apptainer definition、完整 environment lock 或 snapshot-staging record；这是正式模型
smoke 前的明确 blocker。**

首次环境固化必须完成以下闭环：

1. 在允许的最小 GPU allocation 中记录 host driver、`nvidia-smi` 和实际 GPU 型号；
2. 选择与该 driver 兼容的 CUDA/PyTorch base，并把完整 `.def` 纳入 Git；
3. 在 definition 中安装本 commit 的 `.[llm]` 依赖，构建 `.sif`，运行 `pip check`；
4. 保存 definition Git commit、build log、image SHA256 和排序后的 `pip freeze`；
5. 按 config 的 dataset/model/tokenizer commit revision 预缓存全部 HF assets，并保存
   `repo_id/revision/cache digest` inventory；
6. 在 `TRANSFORMERS_OFFLINE=1`、`HF_DATASETS_OFFLINE=1` 下做 tokenizer/config/dataset/model
   resolve check；只有完全离线解析成功才进入 GPU smoke。
...
| SIF + SHA256 + build log | `/project/sigroup/.../images/` |
| `pip check` + sorted `pip freeze` | `system-reports/` |
| HF repo/revision/cache inventory | `/project/sigroup/.../hf-cache/` 旁的 manifest |
| Offline resolve log | `system-reports/` |
| Host driver、partition、GPU model | GPU smoke report |
...
array concurrency 只改变 campaign makespan，不减少总 GPU-hours。
